"""
Calibration script for the Beech stand at AMALIA pilot site.
...
Usage:
    python run_calibration_beech.py <stage>

Stages:
    broad        — full 14-parameter global DE search
    fine         — narrowed DE search from calib_res/best_guess_beech.in
    ...
    subset-finer — L-BFGS-B multi-start from best row in de_log_beech.csv
...
DRUTES_TEMPLATE = "drutes_temp_beech"
...
def runDrutes(strategy, log, par):
    ...
    try:
        ...
        logfile = "finetune_log_beech.csv" if log == "fine" else "de_log_beech.csv"
...
if __name__ == '__main__':
    ...
    S_max_bnd   = (np.log10(1e-9), np.log10(10e-7))
    ...
    # ------------------------------------------------------------------ #
    # BEECH subset configuration                                           #
    # Update FIXED_PARAMS with best "all"-stage values before running     #
    # "subset". Thermal params (b) are less sensitive to water budget      #
    # errors and can be trusted from the broad/fine stages.               #
    # ------------------------------------------------------------------ #
    FIXED_PARAMS = {
        "b2_org": 0.872773,    # update after "all" stage
        "b1_min": 0.0685467,   # update after "all" stage
        "b2_min": 0.854659,    # update after "all" stage
        "b3_min": 1.4491847,   # update after "all" stage
        "n_min":  1.5,         # update after "all" stage; reset from old unreliable value
    }

    b1_org_bnd    = (0.02, 3.0)
    b3_org_bnd    = (0.02, 6.0)
    albedo_bnd    = (0.05, 0.3)
    alpha_org_bnd = (np.log10(1), np.log10(6))
    n_org_bnd_sub = (1.05, 5.0)
    K_org_bnd     = (np.log10(1.0e-8), np.log10(10.0e-4))
    alpha_min_bnd = (np.log10(1), np.log10(5))
    K_min_bnd     = (np.log10(1.0e-8), np.log10(10.0e-4))
    S_max_bnd_sub = (np.log10(1e-9), np.log10(10e-7))
    ...
    match stage:
       ...
       # ---------------------------------------------------------------- #
       case "broad":
           with open("de_log_beech.csv", "a") as f:
               timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
               f.write(f"\n# OPTIMISATION LOG --- BEECH BROAD --- {timestamp}\n"
       ...
       # ---------------------------------------------------------------- #
       # Fine: narrowed search seeded from best_guess_beech.in            #
       # ---------------------------------------------------------------- #
       case "fine":
           best_guess = np.loadtxt("calib_res/best_guess_beech.in")
           ...
           with open("finetune_log_beech.csv", "a") as f:
               timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
               f.write(f"\n# OPTIMISATION LOG --- BEECH FINE --- {timestamp}\n"
                       "call_id,timestamp,error,error_heat,error_moist,"
                       "b1_org[W/(m.K)],b2_org[W/(m.K)],b3_org[W/(m.K)],"
                       "b1_min[W/(m.K)],b2_min[W/(m.K)],b3_min[W/(m.K)],"
                       "albedo[-],"
       ...
       case "subset":
           with open("de_log_beech.csv", "a") as f:
               timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
               f.write(f"\n# OPTIMISATION LOG --- BEECH SUBSET --- {timestamp}\n"
       ...
       case "subset-fine":
           ...
           with open("finetune_log_beech.csv", "a") as f:
               timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
               f.write(f"\n# OPTIMISATION LOG --- BEECH SUBSET-FINE --- {timestamp}\n"
                       "call_id,timestamp,error,error_heat,error_moist,"
                       "b1_org[W/(m.K)],b2_org[W/(m.K)],b3_org[W/(m.K)],"
                       "b1_min[W/(m.K)],b2_min[W/(m.K)],b3_min[W/(m.K)],"
                       "albedo[-],"
       ...
       # ---------------------------------------------------------------- #
       case "subset-finer":
           log_df = pd.read_csv("de_log_beech.csv", comment="#", header=0)
           ...
           with open("finetune_log_beech.csv", "a") as f:
               timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
               f.write(f"\n# OPTIMISATION LOG --- BEECH SUBSET-FINER (L-BFGS-B) --- {timestamp}\n"
                       f"# CALIBRATED VARS: {display_subset}\n"
                       "call_id,timestamp,error,error_heat,error_moist,"
                       "b1_org[W/(m.K)],b2_org[W/(m.K)],b3_org[W/(m.K)],"
                       "b1_min[W/(m.K)],b2_min[W/(m.K)],b3_min[W/(m.K)],"
