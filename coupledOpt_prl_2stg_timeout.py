...
def runDrutes_subset(par_subset):
    """
...
# Generate unique dir name
    ...
    # If it survived, calculate error normally
    try:
        error, error_heat, error_moist = getError(run_dir)
        
        # Log results
        full_par_for_log = [b1_org, b2_org, b3_org, b1_min, b2_min, b3_min, 
                            albedo, alpha_org, n_org, K_org, alpha_min, n_min, K_min, S_max]
        log_run(call_id, error, error_heat, error_moist, full_par_for_log)
    except Exception as e:
        print(f"Error reading results from {run_id}: {e}")
        error = 1e10
    finally:
        shutil.rmtree(run_dir, ignore_errors=True)
